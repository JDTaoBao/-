...
@property (nonatomic, strong)JXTabBar *tabBarView;
...
//    [self.tabBar removeFromSuperview];  //移除TabBarController自带的下部的条
    ...
    //测试添加自己的视图
    self.tabBarView = [[JXTabBar alloc] init]; //设置代理必须改掉前面的类型,不能用UIView
    _tabBarView.backgroundColor = RGBCOLOR(107, 213, 252, 1);
    _tabBarView.delegate = self; //设置代理
    _tabBarView.frame = rect;
    [self.tabBar addSubview:_tabBarView]; //添加到系统自带的tabBar上, 这样可以用的的事件方法. 而不必自己去写
